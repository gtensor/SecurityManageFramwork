    ...
    class Meta:
        ...
        widgets = {
            ...
            'reason': widgets.Textarea(
                attrs={'class': 'form-control', 'placeholder': '资产转让说明（该操作会将您名下的资产转让 给目标账号，并停用该账号，请谨慎操作）'}),
        }
...
class Asset_create_form(ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'asset_key': widgets.TextInput(
                attrs={'class': 'form-control', 'placeholder': '资产标识/服务器为ip，网站为域名或访问地址,APP为应用名称'}),
...
class OS_Info_form(ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'up_time': widgets.DateInput(
                attrs={'class': 'layui-input date', 'placeholder': 'yyyy-MM-dd', 'autocomplete': 'off',
                       'lay-verify': 'date'}),
            'guarante_time': widgets.DateInput(
                attrs={'class': 'layui-input date', 'placeholder': 'yyyy-MM-dd', 'autocomplete': 'off',
                       'lay-verify': 'date'}),
            'down_time': widgets.DateInput(
                attrs={'class': 'layui-input date', 'placeholder': 'yyyy-MM-dd', 'autocomplete': 'off',
                       'lay-verify': 'date'}),
        }


class InternetInfoForm(ModelForm):
    class Meta:
        model = models.InternetInfo
        fields = ['middleware', 'middleware_version', 'is_out', 'out_key', 'web_status', 'language', 'language_version',
        ...
        widgets = {
            ...
            'web_framework': widgets.TextInput(attrs={'class': 'form-control', 'placeholder': '开发框架'}),
            'web_framework_version': widgets.TextInput(attrs={'class': 'form-control', 'placeholder': '框架版本'}),
        }
